...
bool askQuestion(char positive, char negative) {
    ...
    while (sign != positive && sign != negative) {
        std::cout << "Неправильный ввод: '" << sign << "', должно быть либо" << positive << " либо " << negative << "\n";
        std::cin >> sign;
        sign = tolower(sign);
    }
...
void processPlayerTurn(GameField& field, Sign player_sign) {
    std::cout << "Введите ряд и столбик: ";
    ...
    while (!isValidCell(row - 1, column - 1) || !isCellEmpty(field, row - 1, column - 1)) {
        ...
        std::cout << "Неправильный ввод, должна быть пустая ячейка: ";
        std::cin >> row >> column;
    }
...
bool queryPlayAgain() {
    std::cout << "Жотите сыграть еще раз? [y или n]: ";
...
void printGameOutcome(const TurnOutcome& outcome, Sign player_sign) {
    if (outcome.isDraw) {
        ...
        std::cout << "Поздравляю! Ты победитель.\n";
    } else {
        std::cout << "Не повезло.\n";
    }
}
